    ...
    %TypeRelUnit%, Float%TypeRel%, Float%TypeRel%Vector>
{
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, Float%TypeRel%> */
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, Float%TypeRel%&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit %TypeRelUnit%; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public Float%TypeRel%Vector(final SortedMap<Integer, ? extends Number> data, final int size, final %TypeRelUnit% displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof Float%TypeRel%
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public Float%TypeRel%Vector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public Float%TypeRel%Vector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
